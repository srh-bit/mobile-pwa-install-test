...
GitHub Actions is the repository acceptance path. Debug APK output is validation evidence only and is not published or treated as a production package.

## Release inputs

`HRFH_ANDROID_APPLICATION_ID` overrides the staging application ID. `HRFH_TWA_ORIGIN` controls the fixed trusted origin and defaults to `https://myhrfh.com`.
...
The package accepts no caller-selected origin, URL, package, Intent, command, or shell input. Digital Asset Links `delegate_permission/common.use_as_origin` must validate before the postMessage channel can become ready.
